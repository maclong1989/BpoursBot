...
import org.bpours.service.AnswerService;
...
public class QQService {
		...
		xiaoV = new SmartQQClient(new MessageCallback() {
			...
			public void onGroupMessage(final GroupMessage message) {
				...
				if (answerService == null) {
					answerService = SpringManager.getBean(AnswerService.class);
				}
	...
	private void sendMessageToGroup(final Long groupId, final String msg) {
		...
		if (msg.contains("<br/>")) {
			...
			for (String aMasg : msgs) {

				if (StringUtils.isBlank(aMasg)) {
					continue;
				}

				LOGGER.info("Pushing [msg=" + aMasg + "] to QQ qun [" + group.getName() + "]");
				xiaoV.sendMessageToGroup(groupId, aMasg);

				try {
					Thread.sleep((RandomUtils.nextInt(50) + 100l) * 20l);
	...
	private void onQQGroupMessage(final GroupMessage message) {
		...
		LOGGER.info("收到群消息:内容:" + content + ",用户" + userName);
	...
	private String answer(final String content, final String userName) {
		...
		ret = answerService.answer(content, userName);

		if (StringUtils.isEmpty(ret) && StringUtils.contains(content, XiaoVs.QQ_BOT_NAME)) {
			if (1 == QQ_BOT_TYPE) {
